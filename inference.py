#modified from https://github.com/microsoft/table-transformer/blob/main/src/inference.py
from custom import MyStandardROIHeads, MyStandardRPNHead, MyGeneralizedRCNN, MyFastRCNNConvFCHead, MyAnchorGenerator
...
#ocr = MMOCR(det_config="/data/logs/mmocr/lytica_only_fcenet_log_new/fcenet_r50dcnv2_fpn_1500e_lytica.py", det_ckpt="/data/logs/mmocr/lytica_only_fcenet_log_new/latest.pth",  recog_config='/data/logs/mmocr/lytica_syn_sar_log/sar_r31_parallel_decoder_lytica.py', recog_ckpt='/data/logs/mmocr/lytica_syn_sar_log/latest.pth')
#ocr = MMOCR(det="FCE_CTW_DCNv2",  recog_config='/data/logs/mmocr/lytica_syn_sar_log/sar_r31_parallel_decoder_lytica.py', recog_ckpt='/data/logs/mmocr/lytica_syn_sar_log/latest.pth')
...
def build_model(config_path, model_path, device):
...
def outputs_to_objects(outputs, img_size, class_idx2name):
    ...
    pred_bboxes = outputs["pred_boxes"].tensor.detach().cpu()
    pred_labels = outputs["pred_classes"].detach().cpu()
...
def resize_pad(image, target_size):
    ...
    #target_size = get_preprocess_shape(h, w, target_size)
    #image = cv2.resize(image, target_size)

    # Pad image and mask to form a square
    h, w = image.shape[0], image.shape[1]
    #max_dim = max(w, h)
    max_dim = max(target_size, w, h)
    pad_h = (max_dim - h) // 2
    pad_w = (max_dim - w) // 2
    padding = (pad_w, pad_h, max_dim - w - pad_w, max_dim - h - pad_h)

    image = cv2.copyMakeBorder(image, pad_h, max_dim - h - pad_h, pad_w, max_dim - w - pad_w, cv2.BORDER_CONSTANT, None, value=(255,255,255))

    return image



def extract_text_inside_bbox(image, bbox):

    x1, y1, x2, y2 = int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])
    cell_image = image[y1:y2, x1: x2]

    #cell_image = cv2.copyMakeBorder(cell_image, 50, 50, 50, 50, cv2.BORDER_CONSTANT, None, value=(255,255,255))
    cell_image = resize_pad(cell_image, 600)

    #cv2.imwrite("cell_image.jpg", cell_image)
    #exit()


    #ocr_result_list = ocr.readtext(cell_image)
    result_str = ""
    #for ocr_result in ocr_result_list:
    #    text_str_list = ocr_result["text"]
    #    for text_str in text_str_list:
    #        result_str = result_str + " " + text_str.replace("<UKN>", " ")

    return result_str

def structure_to_cells(image, table_structure, tokens):
    """
    ...
    for column_num, column in enumerate(columns):
        for row_num, row in enumerate(rows):
            ...
            for spanning_cell in spanning_cells:
                ...
                if cell_rect.get_area() != 0 and (spanning_cell_rect.intersect(cell_rect).get_area()
            ...
            if cell['subcell']:
                ...
                #cell_text = extract_text_inside_bbox(image, cell['bbox'])
    ...
    #for cell in cells:
    #    bbox = cell["bbox"]
    #    cell_text = extract_text_inside_bbox(image, bbox)
    #    cell["cell text"] = cell_text
...
class TableExtractionPipeline(object):
    def __init__(self, det_device=None, str_device=None,
        ...
        if not det_config_path is None:
            ...
            self.det_model = build_model(det_config_path, det_model_path, det_device)
        ...
        if not str_config_path is None:
            ...
            self.str_model = build_model(str_config_path, str_model_path, str_device)
    ...
    def recognize(self, img, word_data, tokens=None, out_objects=False, out_cells=False,
        ...
        # Post-process detected objects, assign class labels
        objects = outputs_to_objects(output_dict, image_size, self.str_class_idx2name)
        if out_objects:
            out_formats['objects'] = objects
        ...
        tables_cells = [structure_to_cells(img, structure, tokens)[0] for structure in tables_structure]
        if out_cells:
