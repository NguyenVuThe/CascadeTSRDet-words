...
__all__ = ["MyGeneralizedRCNN", "ProposalNetwork"]
...
class MyGeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        #_, detector_losses = self.roi_heads(images, features, proposals, context_proposals, gt_instances)
    ...
    def inference(
        ...
        if detected_instances is None:
            if self.proposal_generator is not None:
                proposals, context_proposals = self.proposal_generator(images, features, None)
            else:
                assert "proposals" in batched_inputs[0]
                proposals = [x["proposals"].to(self.device) for x in batched_inputs]
